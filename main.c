...
/*   Updated: 2026/01/06 18:56:52 by yseto            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_test_textures(t_game *state)
{
	...
	while (i < 4)
	{
		...
		tex->data_addr = mlx_get_data_addr(
				tex->img_ptr,
				&tex->bpp,
				&tex->line_len,
				&tex->endian);
...
int	main(void)
{
	...
	state.mlx.win_ptr = mlx_new_window(
			state.mlx.mlx_ptr,
			state.mlx.win_width,
	...
	state.mlx.img_ptr = mlx_new_image(
			state.mlx.mlx_ptr,
			state.mlx.win_width,
			state.mlx.win_height);
	state.mlx.img_addr = mlx_get_data_addr(
			state.mlx.img_ptr,
			&state.mlx.bpp,
			&state.mlx.line_len,
			&state.mlx.endian);
