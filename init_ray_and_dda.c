...
/*   Updated: 2025/12/15 18:19:36 by yseto            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_step_x(t_game *state, t_ray *ray)
{
	if (ray->dir_x < 0)
	{
		ray->step_x = -1;
		ray->side_dist_x =
			(state->player.pos_x - ray->map_x) * ray->delta_dist_x;
	}
	else
	{
		ray->step_x = 1;
		ray->side_dist_x =
			(ray->map_x + 1.0 - state->player.pos_x) * ray->delta_dist_x;
	}
}
...
static void	init_step_y(t_game *state, t_ray *ray)
{
	if (ray->dir_y < 0)
	{
		ray->step_y = -1;
		ray->side_dist_y =
			(state->player.pos_y - ray->map_y) * ray->delta_dist_y;
	}
	else
	{
		ray->step_y = 1;
		ray->side_dist_y =
			(ray->map_y + 1.0 - state->player.pos_y) * ray->delta_dist_y;
	}
}
